#SUPER SMASH BROS
#ICS3U-FSE
#SAJAN FLORA, SEHAJ CHEEMA, ALEX STEFANOVSKI
#FINAL ASSIGNMENT
#The object of the game is to knock the opponent
#off the platform 3 times in order to win. The
#knockback percentage is shown on the top of the
#screen as well as how many times you have fallen
#off the map. You have a choice of playing against
#another player or the computer. When choosing a
#character you can randomize and after choosing
#you must confirm your choice by clicking on the confirm
#button. Once picked you are able to choose 1 out of the 5
#stadiums to play in. Make sure your volume is on so you can
#hear the special sound effects. Credits shows the project
#creators. Instructions show keybinds for each player.
#Player 1 starts on the left side of the screen and Player
#2 starts on the right side of the screen.
#---------------------------------------------------------------
from pygame import *
from random import *
from math import *
...
char=["Mario","Mario"] #Characters
movel=[6,6] #Move left
mover=[12,12] #Move right
lobby=False 
confirm=[False,False] #To confirm the character choices
stadium=("Unchosen") #Determines which stadium is choosen
cond=False #Helps with QUIT button bug
...
right=[4,4]  #counter for ball attack facing right
left=[4,4]  #counter for ball attack facing left
x=0   #x value
y=1   #y value
vy=2    #gravity
ground=3   #if the player is on the ground
direction=4    #what direction the charcter is facing
...
frame1=0#current frame within the move for player 1
frame2=0 #current fram within the move for player 2
move1=0 #current move for player 1
move2=0 #current move for player 2
        ...
        if cond:
            ...
            if lobby==True and confirm[0]==False: #PLAYER 1, character isnt confirmed
                if Character[0]=="Mario": #Indicates Character choosen
                    ...
                    movel[0]=6              #used for sprites facing left
                    mover[0]=12             #used for sprites facing right
...
#_____________________p1__________________________________________           
    ...
    if click[0]==True:
        ...
        if attack[0]>n[x]-5 and attack[0]<n[x]+5:  #if the ball is in contact with the opponent
            if p1[y]+8>n[y]-29 and p1[y]+8<n[y]+29  and attack2[3]==False:    #if block is false and 
                if attack[0]-n[x]>0:  
